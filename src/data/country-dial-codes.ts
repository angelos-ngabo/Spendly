export type CountryDial = {
  iso: string
  name: string
  /** International dial prefix including + */
  dial: string
}

/** Sorted by country name. Dial codes match common Firebase / web forms usage (ITU-style). */
...
export function countryByIso(iso: string): CountryDial | undefined {
  return COUNTRY_DIAL_CODES.find((c) => c.iso === iso)
}

/** Guess default country ISO from `navigator.language` (e.g. en-US → US). */
export function defaultCountryIso(): string {
  if (typeof navigator === 'undefined') return 'US'
  const region = navigator.language?.split('-')[1]?.toUpperCase()
  if (region && countryByIso(region)) return region
  return 'US'
}
